...
const nowPlaying = baseUrl + "/movie/now_playing?language=en-US&page=1";
...
export const getNowPlaying = async (): Promise<MovieList> => {
  const response = await fetch(nowPlaying, options);
